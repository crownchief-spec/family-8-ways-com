...
<section class="container section admin-wrap admin-edit-page">
  ...
  <article class="card admin-edit-fallback" id="admin-md-fallback-panel" hidden>
    ...
    <p class="muted">可直接<strong>下載</strong>後覆蓋本機 <code id="admin-md-path-hint"></code>；或用全選複製貼回 Cursor／編輯器。</p>
    ...
    <div class="admin-actions admin-edit-fallback-actions">
      <button type="button" class="btn btn--primary" id="admin-download-md-output">下載 .md 檔案</button>
      <button type="button" class="btn btn--secondary" id="admin-copy-md-output">複製 Markdown</button>
    </div>
  </article>
</section>`;
}
