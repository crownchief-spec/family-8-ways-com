...
export function buildClientPortalBody(c, site, { escapeHtml, isPresent }) {
  ...
  const formArticle = readyShare
    ...
    <form id="client-contract-form" novalidate>
      ...
      <label class="check-row"><input type="checkbox" name="confirmBookingInfo" required /> 我已確認以上拍攝日期、時間、地點、方案、費用與成品內容。</label>
      <label class="check-row"><input type="checkbox" name="confirmPhotographerUpdate" required /> 我知道若以上拍攝資訊需要調整，需由攝影師更新後台資料。</label>
      <label class="check-row"><input type="checkbox" name="confirmPaymentStatus" required /> 我已填寫目前訂金付款狀態。</label>
      ...
      <label class="check-row"><input type="checkbox" name="confirmSignature" required /> 我確認以上客戶補充資料正確，並同意以電子簽名方式完成本次預約確認。</label>
      <div class="portal-grid-2">
